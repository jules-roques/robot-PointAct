...
**The grid.** {OpenDrawer, CloseBlenderLid, PickPlaceCounterToStove} × {uniform, eef, oracle} ×
{512, 1024, 2048, 4096, 8192, 16384}, plus one no-sampler arm per task = **57 arms, 30K steps,
checkpoints every 5K** (5K…30K). Generated by `runs/generate_stage7.py`, submitted by
`submit_stage7.sh`.

OpenDrawer went first (2026-09-10); CloseBlenderLid and PickPlaceCounterToStove were added
2026-09-21. **The three curves are read separately and never pooled.** Stage 5 showed the
sampler ordering is task-dependent — eef wins on four tasks and loses 8.4 pp on CoffeeSetupMug
— so a budget curve averaged over tasks would average two different shapes and hide exactly the
effect the extra tasks were added to expose. What the second and third task buy is whether the
*shape* of the curve (where the knee sits, where the samplers converge) is a property of point
policies or a property of OpenDrawer.

These two tasks were chosen because they **bracket stage 5's sampler spread**. On
PickPlaceCounterToStove the prior decides almost everything (uniform 4% against eef 51%), so
its knee should be the most violent in the grid; CloseBlenderLid is the flatter, large-target
case. CoffeeSetupMug and TurnOnMicrowave are left out to hold the grid at three tasks — mug is
the one task where eef *loses*, which makes it the obvious fourth if budget allows.

No data preparation was needed: both tasks were built for stage 5, so their point LMDB, cached
text context, `roi_meta/target_positions.npz` (the oracle arm's GT) and `rot6d.json` norm stats
were already on disk, and `geom_gt.ORACLE_TARGET` already maps them to `lid` and `obj`.
Because `eval_task_jeanzay.slurm` refuses a mixed-task `RUNS` list, evaluation is **six
submissions, two per task**, not two in total.

**Why 30K and not 50K** (set 2026-09-10). Budget first: 50K put the grid at ~795 H100-h against
~3K remaining, and 30K brings it to ~511. It is also well supported — the stage 1–3 duration
curves showed every arm effectively converged by 30K, which is why stage 5 chose 30K — and it
makes these arms **directly comparable to `s5-od-*-n8192-s0`**, already trained at 30K on this
same Utonia recipe, which is a free replication check that 50K did not offer.

**The schedule stays cosine, deliberately.** A horizon-free schedule (`warmup_stable_decay`,
available in the installed transformers 5.5.4, or constant LR + weight averaging) would let an
undertrained arm *resume* rather than restart, and would make every intermediate checkpoint
properly annealed. It was rejected on tuning cost: the stable LR is not cosine's peak, so
switching means re-tuning, and there is no budget for that. Revisit when the tooling is ready.
The price is that **an intermediate checkpoint is "where a 30K run was at step N", not "a policy
trained for N steps"** — the LR is still ~97% of peak at 5K, ~50% at 15K, ~9% at 25K, so the
early points are systematically pessimistic about their own budget (Chinchilla, App. B). Only
the 30K column is annealed; say so wherever the duration curve is plotted.
...
**The no-sampler arm is trained fresh, and the 30K decision is why.** The plan until
2026-09-10 was to reuse stage 6's `od-none-s0`, already at 50K on this exact recipe. The
shorter horizon kills that reuse: its checkpoint-30000 sits mid-cosine at ~36% of peak LR — a
snapshot of a 50K run, not a 30K-annealed policy — while its 50K endpoint has seen 1.67× the
optimiser steps of everything it would be compared against. Neither is a fair no-sampler point
on a curve of 30K-annealed arms, so `s7-od-none-s0` is trained at 30K for ~34 H100-h. Stage 6's
arm remains valid on its own terms.

**Evaluation: 100 at every checkpoint, 500 at the end.** One seed × 100 trials at each of
5K…30K gives the duration curve; four more seeds at 30K pool with it to n=500 for the headline
...
**Cost — measured 2026-09-08, job 1880350, and the rate is ~2x what the stage-1 table implies.**
**~511 H100-h of training at 30K** (18 sampled arms ~477 + the no-sampler arm ~34), plus ~11
node-hours of eval for ~17,100 rollouts at the measured ~1600 trials/h/node. Eval is a rounding
error against training, and it runs on A100 rather than H100.

Marginal s/step on 4×H100 at effective batch 128 (`pilot_throughput.py`, 40→200 step window).
GPU-h is given for the 30K budget actually being run; the s/step column is what was measured:

| budget | s/step | 30K wall | GPU-h @30K |
|---|---|---|---|
| 1024 | 0.725 | 6.0 h | 24.2 |
| 2048 | 0.704 | 5.9 h | 23.5 |
| 4096 | 0.835 | 7.0 h | 27.8 |
| 8192 | 0.798 | 6.7 h | 26.6 |
| 16384 | 0.985 | 8.2 h | 32.8 |

Every arm fits the 20 h `qos_gpu_h100-t3` cap with room to spare, so none needs t4. **The rate is ~2.7x the
