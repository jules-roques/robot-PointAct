...
| 7 | `Stage 7: Point budget x sampler (Utonia)` | the nineteen `s7-od-*` | **the point-budget axis, re-measured on an encoder whose pretraining granularity matches ours** |
...
**The grid.** OpenDrawer × {uniform, eef, oracle} × {512, 1024, 2048, 4096, 8192, 16384},
plus a no-sampler arm = **19 arms, 30K steps, checkpoints every 5K** (5K…30K). Generated by
`runs/generate_stage7.py`, submitted by `submit_stage7.sh`. One task on purpose: stage 5 showed
the sampler ordering is task-dependent (eef wins on four tasks and loses 8.4 pp on
CoffeeSetupMug), so a curve pooled across tasks would average two different shapes.
