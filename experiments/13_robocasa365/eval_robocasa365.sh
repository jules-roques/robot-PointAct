...
# Resolve the repository root from this script's own location (it is invoked as a plain
# script, not submitted directly), so the checkout can live anywhere.
...
# On any other cluster, set FFMPEG_LIB_DIR to an ffmpeg 6.x lib directory.
if [ -z "${DSDIR:-}" ]; then
